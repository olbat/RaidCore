...
mod:RegisterEnableMob("Ohmna la Terriphage") -- Dreadphage Ohmna
...
function mod:OnUnitCreated(unit)
	...
	if sName == "Tentacule d'Ohmna" then -- Tentacle of Ohmna
		if not OhmnaP4 then
			core:AddMsg("OTENT", "Tentacles", 5, "Info", "Blue")
			core:AddBar("OTENT", "Next Tentacles", 20)
		end
	elseif sName == "Gueule vorace de la Terriphage" then -- Ravenous Maw of the Dreadphage
		core:MarkUnit(unit, 0)
		core:AddLine(unit:GetId(), 2, unit, nil, 3, 25, 0)
	elseif sName == "Ohmna la Terriphage" then -- Dreadphage Ohmna
		core:AddUnit(unit)
	end
end
...
function mod:OnSpellCastStart(unitName, castName, unit)
	if unitName == "Ohmna la Terriphage" and castName == "Jaillir" then -- Dreadphage Ohmna, Erupt
		...
		core:StopScan()
	elseif unitName == "Ohmna la Terriphage" and castName == "Torrent génétique" then -- Dreadphage Ohmna, Genetic Torrent
...
function mod:OnCombatStateChanged(unit, bInCombat)
	if unit:GetType() == "NonPlayer" and bInCombat then
		local sName = unit:GetName()
		if sName == "Ohmna la Terriphage" then -- Dreadphage Ohmna
			self:Start()
