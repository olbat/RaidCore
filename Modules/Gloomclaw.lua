...
mod:RegisterEnableMob("Serrenox") -- Gloomclaw
...
function mod:OnUnitCreated(unit)
	...
	--Print(sName .. " Created")
	--[[
	if sName == "Infosphère de bassin de corruption" then -- Datascape Corruption Pool
		core:MarkUnit(unit)
	end--]]
	if sName == "Ravageur corrompu" or sName == "Empowered Ravager" then -- Corrupted Ravager, Empowered Ravager
		core:WatchUnit(unit)
	elseif sName == "Déchiqueteur instable" then -- Volatile Mauler
...
function mod:OnSpellCastStart(unitName, castName, unit)
	...
	if unitName == "Serrenox" and castName == "Rupture" then -- Gloomclaw, Rupture
...
function mod:OnCombatStateChanged(unit, bInCombat)
	if unit:GetType() == "NonPlayer" and bInCombat then
		...
		if sName == "Serrenox" then -- Gloomclaw
			self:Start()
			waveCount, ruptCount, prev = 0, 0, 0
			section = 1
			first = true
